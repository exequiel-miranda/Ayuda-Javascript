...
(function($) {
    ...
    var api = {
        ...
        open: function( speed, overlap, easing ) {
            ...
            $kids.each( function( index, el ) {
                ...
                $over.css(utils.prefix({
                    'animation': 'unfold-over ' + (speed * 0.10) + 's ' + easing + ' ' + wait + 's 1 normal forwards'
                }));
            });

            // Add momentum to the container
            $root.css(utils.prefix({
                'animation': 'swing-out ' + ( $kids.length * time * 0.5 ) + 's ease-in-out 0s 1 normal forwards'
            }));
        ...
        close: function( speed, overlap, easing ) {
            ...
            // Establish values or fallbacks
            speed = utils.resolve( $this, 'speed', speed ) * 0.15;
            easing = utils.resolve( $this, 'easing', easing );
            overlap = utils.resolve( $this, 'overlap', overlap );
            ...
            $kids.each( function( index, el ) {
                ...
                if ( !last ) wait = ( ( $kids.length - index - 2 ) * time ) + ( speed * 0.15 );

                // Shading animation
                $over.css(utils.prefix({
                    'animation': 'fold-over ' + (speed * 0.15) + 's ' + easing + ' ' + wait + 's 1 normal forwards'
                }));
            });

            // Add momentum to the container
            $root.css(utils.prefix({
                'animation': 'swing-in ' + ( $kids.length * time * 0.2 ) + 's ease-in-out 0s 1 normal forwards'
            }));
